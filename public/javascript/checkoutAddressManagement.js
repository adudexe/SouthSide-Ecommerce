...
document.getElementById('submitButton').addEventListener('click', async function (event) {
    ...
    // Validate Postal Code: should be a number and not empty
    if (isNaN(newAddress.postalCode) || newAddress.postalCode.trim() === "") {
        Swal.fire({
            icon: 'error',
            title: 'Validation Error',
            text: 'Postal Code must be a valid number'
        });
        return;
    }

    // Validate Phone Number: simple check for a 10-digit number
    ...
    // If all validations pass, proceed to add the address
    try {
        const response = await fetch("/user/checkout/address/add", {
            method: "POST",
            headers: {
...
document.getElementById("updateSubmitButton").addEventListener("click", async (e) => {
    ...
    if (fields.postalCode.trim() === "" || isNaN(fields.postalCode)) {
        isValid = false;
        validationErrors.push("Postal Code is required and must be a valid number!");
    }

    const phonePattern = /^[0-9]{10}$/;
    if (fields.phone.trim() === "" || !phonePattern.test(fields.phone)) {
        isValid = false;
        validationErrors.push("Phone number is required and must be 10 digits!");
    }
    ...
    // If there are validation errors, show SweetAlert and return
    if (!isValid) {
        Swal.fire({
            icon: 'error',
            title: 'Oops...',
    ...
    // Check if any field has changed compared to originalAddress
    for (let key in fields) {
        if (key !== 'addressId' && fields[key] !== window.originalAddress[key]) {
            isChanged = true;
            break;
        }
    }

    // If nothing changed, show SweetAlert and return
    if (!isChanged) {
        Swal.fire({
            icon: 'warning',
            title: 'No Changes Detected',
    ...
    // Proceed with the update request if everything is valid and changes are detected
    try {
        let response = await fetch(`/user/checkout/address/update/${window.currentAddressId}`, {
            method: "PUT",
            headers: {
        ...
        if (response.ok) {
            ...
            Swal.fire({
                ...
                // Optionally reload the page to show updated address
                location.reload();
            });
        } else {
            // Handle failure from the backend (e.g., server error or validation error)
            Swal.fire({
                icon: 'error',
                title: 'Update Failed',
                text: result.message || 'There was an issue updating your address. Please try again.'
